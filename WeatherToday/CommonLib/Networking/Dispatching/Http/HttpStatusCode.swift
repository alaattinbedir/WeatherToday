...
public enum HTTPStatusCode: Int, Error, Codable {
    ...
    public enum ResponseType {
        ...
        public var isErrorType: Bool {
            self == .clientError || self == .serverError || self == .undefined
        }
    }
    ...
    /// - multiStatus: The message body that follows is an XML message and can contain a number of separate response codes,
    ...
    /// - IMUsed: The server has fulfilled a request for the resource,
    ...
    /// - methodNotAllowed: A request method is not supported for the requested resource. e.g. a
    /// GET request on a form which requires data to be presented via POST
